...
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "internal_apps_config" {
  ...
  config {
    ingress_rule {
      ...
      service  = "http://sase-grafana:3000"
    }
    ...
    ingress_rule {
      ...
      service  = "http://sase-keycloak:8080"
    }
...
# DNS records pointing subdomains to the tunnel
...
resource "cloudflare_record" "wiki" {
  ...
  name    = "wiki"
  content = "${cloudflare_zero_trust_tunnel_cloudflared.internal_apps.id}.cfargotunnel.com"
  type    = "CNAME"
  proxied = true
}
...
resource "cloudflare_record" "topology" {
  zone_id         = data.cloudflare_zones.domain.zones[0].id
  name            = "topology"
  content         = "${cloudflare_zero_trust_tunnel_cloudflared.internal_apps.id}.cfargotunnel.com"
  type            = "CNAME"
...
# NOTE: Only enable after cloudflared tunnel is running and keycloak.sase-lab.win is reachable
# Uses explicit endpoint URLs because the Cloudflare API v4 requires them separately
resource "cloudflare_zero_trust_access_identity_provider" "keycloak" {
  count      = var.enable_keycloak_idp ? 1 : 0
  account_id = var.cloudflare_account_id
  name       = "Keycloak"
  type       = "oidc"
  config {
    ...
    auth_url      = "${var.keycloak_issuer_url}/protocol/openid-connect/auth"
    token_url     = "${var.keycloak_issuer_url}/protocol/openid-connect/token"
    certs_url     = "${var.keycloak_issuer_url}/protocol/openid-connect/certs"
    pkce_enabled  = true
...
# Zero Trust Access Application for SASE Architecture Console
